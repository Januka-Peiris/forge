...
| Later | Unified task model | Unify run/chat/coordinator/background units under one task lifecycle model | Simplifies lifecycle handling and cross-surface observability | L | M-H | Forge | Backlog |
| Later | Memory v2 | Add lightweight automatic memory extraction + selective recall | Better long-session continuity without heavyweight cloud features | L | M | Forge | Backlog |
| Later | Scheduler evolution | Expand orchestrator scheduling to per-workspace durable schedules/jitter/locks | Enables safer automation and scalable background coordination | M-L | M | Forge | Backlog |
