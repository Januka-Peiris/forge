        ...
        <div className="ml-auto flex flex-wrap items-center gap-1.5">
          {unattached.slice(0, 2).map((terminal) => (
            <button
              ...
              className="rounded-md border border-forge-blue/25 bg-forge-blue/10 px-2 py-1 text-[10px] font-semibold text-forge-blue hover:bg-forge-blue/15 disabled:opacity-50"
            >
          ...
          {failed.slice(0, 2).map((terminal) => (
            <button
              ...
              className="rounded-md border border-forge-border bg-white/5 px-2 py-1 text-[10px] font-semibold text-forge-muted hover:bg-white/10 disabled:opacity-50"
            >
    ...
    <div className="mt-3 rounded-lg border border-forge-border/70 bg-white/[0.03] px-3 py-2">
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex min-w-0 items-center gap-2 text-[11px]">
          ...
          {runningRunCount > 0 && (
            <span className="rounded-full border border-forge-blue/25 bg-forge-blue/10 px-2 py-0.5 text-[10px] text-forge-blue">{runningRunCount} run active</span>
          )}
        </div>
        <div className="ml-auto flex flex-wrap items-center gap-1.5">
          ...
          {config.run.map((command, index) => (
            <div key={`${index}-${command}`} className="flex items-center gap-1 rounded-md border border-forge-border/70 bg-forge-bg px-1.5 py-1">
              <button
                ...
                title={command}
                className="max-w-[220px] truncate rounded px-1.5 py-0.5 text-[10px] font-semibold text-forge-orange hover:bg-forge-orange/10 disabled:opacity-50"
              >
              ...
              <button
                ...
                className="rounded px-1.5 py-0.5 text-[10px] text-forge-muted hover:bg-white/10 disabled:opacity-50"
              >
      ...
      {config.warning && <p className="mt-2 text-[11px] text-forge-yellow">{config.warning}</p>}
    ...
    <div className="mt-2 rounded-lg border border-forge-border/70 bg-white/[0.025] px-3 py-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          disabled={busy}
          onClick={onRefresh}
          className="ml-auto rounded-md border border-forge-border bg-white/5 px-2 py-1 text-[10px] font-semibold text-forge-muted hover:bg-white/10 disabled:opacity-50"
        >
      ...
      {ports.length > 0 ? (
          ...
          {ports.map((port) => (
            <div key={`${port.pid}-${port.port}`} className="flex items-center gap-1 rounded-md border border-forge-border/70 bg-forge-bg px-1.5 py-1">
              ...
              <button
                disabled={busy}
                onClick={() => onKill(port)}
                className="rounded px-1.5 py-0.5 text-[10px] text-forge-red hover:bg-forge-red/10 disabled:opacity-50"
        ...
        <p className="mt-1 text-[11px] text-forge-muted">Start a dev server from this workspace, then refresh to open or stop it here.</p>
      )}
    </div>
  );
}
