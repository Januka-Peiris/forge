...
export function CockpitLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-w-0 items-center justify-between gap-3 rounded-lg bg-black/15 px-2.5 py-1.5">
      <span className="shrink-0 text-mn-muted">{label}</span>
      <span className="min-w-0 truncate text-right font-medium text-mn-text/90" title={value}>
        {value}
      </span>
    </div>
  );
...
function taskStatusTone(status: string): string {
  ...
  if (normalized === 'running' || normalized === 'pending') return 'border-mn-blue/20 bg-mn-blue/10 text-mn-blue';
  if (normalized === 'failed' || normalized === 'killed') return 'border-mn-red/20 bg-mn-red/10 text-mn-red';
  if (normalized === 'completed' || normalized === 'succeeded') return 'border-mn-cyan/20 bg-mn-cyan/10 text-mn-cyan';
...
function TaskCenterPanel({
  ...
  if (!snapshot || snapshot.runs.length === 0) {
    return (
      <div className="mt-2 rounded border border-mn-border/60 bg-black/10 px-2 py-2">
        <p className="text-xs font-semibold text-mn-text/85">Task Center</p>
        <p className="mt-0.5 text-xs text-mn-muted">No task runs recorded for this workspace yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-2 rounded border border-mn-border/60 bg-black/10 p-2">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold text-mn-text/85">Task Center</p>
          <p className="mt-0.5 text-xs text-mn-muted">Running work, recent outcomes, and why this workspace may still be waiting.</p>
        </div>
        <span className="text-xs text-mn-muted">{snapshot.runs.length} run(s) · {snapshot.events.length} event(s)</span>
      </div>
      ...
      {waitingReason && (
        <div className="mt-2 flex items-start gap-2 rounded border border-mn-yellow/20 bg-mn-yellow/10 px-2 py-1.5 text-xs text-mn-yellow">
      ...
      <div className="mt-2 space-y-1.5">
        {snapshot.runs.slice(0, 6).map((run) => {
          ...
          return (
            <div key={run.id} className="rounded border border-mn-border/50 bg-black/15 px-2 py-1.5">
              <div className="flex flex-wrap items-center gap-2">
                <span className="min-w-0 flex-1 truncate text-xs font-semibold text-mn-text/90">
                ...
                {run.sourceId && (
                  <span className="shrink-0 rounded border border-mn-border/50 bg-white/5 px-1.5 py-0.5 text-[10px] text-mn-muted">
                    source {run.sourceId}
                  </span>
                )}
              </div>
              <div className="mt-1 grid grid-cols-1 gap-1 text-[11px] text-mn-muted md:grid-cols-2">
              ...
              {payloadSummary && (
                <p className="mt-1 text-[11px] text-mn-text/70">{payloadSummary}</p>
              )}
            </div>
          );
        })}
        {snapshot.runs.length > 6 && <p className="text-xs text-mn-muted">+{snapshot.runs.length - 6} more run(s)</p>}
      </div>

      {snapshot.events.length > 0 && (
        <div className="mt-2 rounded border border-mn-border/50 bg-black/15 p-2">
          <p className="text-xs font-semibold text-mn-text/85">Recent task events</p>
          <div className="mt-1 space-y-1">
            {snapshot.events.slice(0, 5).map((event) => (
              <div key={event.id} className="flex items-start justify-between gap-2 text-xs">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-mn-text/85">{humanizeTaskLabel(event.eventType)}</p>
                ...
                <span className="shrink-0 text-[11px] text-mn-muted">{formatTaskTimestamp(event.ts)}</span>
              </div>
            ))}
          </div>
        </div>
...
export function ChecksShippingPanel({
  ...
  config: MnemonicWorkspaceConfig | null;
  ...
  const checkTone = (check: { status: string; conclusion?: string | null }) => {
    const value = `${check.status} ${check.conclusion ?? ''}`.toLowerCase();
    if (value.includes('fail') || value.includes('error') || value.includes('cancel')) return 'border-mn-red/20 bg-mn-red/10 text-mn-red';
    if (value.includes('pending') || value.includes('progress') || value.includes('queued')) return 'border-mn-yellow/20 bg-mn-yellow/10 text-mn-yellow';
    if (value.includes('success') || value.includes('pass') || value.includes('complete')) return 'border-mn-cyan/20 bg-mn-cyan/10 text-mn-cyan';
    return 'border-mn-border bg-white/5 text-mn-muted';
  };
  ...
  return (
    <div className="px-4 pb-4">
      <div className="rounded-xl border border-mn-border bg-mn-card/70 p-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          <p className="text-xs font-semibold uppercase tracking-widest text-mn-muted">Checks & Shipping</p>
        ...
        {prStatus?.found && prStatus.url && (
          <div className="mt-3 flex items-center justify-between gap-2 rounded-lg border border-mn-border/60 bg-black/10 px-2 py-1.5">
            <div className="min-w-0">
              <p className="truncate text-xs font-semibold text-mn-text/85" title={prStatus.title ?? prLabel}>
                {prStatus.title ?? prLabel}
              </p>
              <p className="text-xs text-mn-muted">{prLabel} · {prStatus.checksSummary}</p>
            </div>
        ...
        {prStatus?.found && prChecks.length === 0 && (
          <div className="mt-3 rounded-lg border border-mn-border/60 bg-black/10 px-2 py-2">
            <p className="text-xs font-semibold text-mn-text/85">GitHub checks</p>
            <p className="mt-0.5 text-xs text-mn-muted">No CI reported for this PR yet.</p>
          </div>
        )}
        {prChecks.length > 0 && (
          <div className="mt-3 rounded-lg border border-mn-border/60 bg-black/10 p-2">
            <div className="mb-1.5 flex items-center justify-between gap-2">
              <p className="text-xs font-semibold text-mn-text/85">GitHub checks</p>
              <span className="text-xs text-mn-muted">failed/pending first · {prChecks.length} reported</span>
            </div>
            <div className="space-y-1">
              {prChecks.slice(0, 5).map((check) => {
                const label = check.conclusion ? `${check.status} · ${check.conclusion}` : check.status;
                return (
                  <div key={`${check.name}-${check.status}-${check.conclusion ?? ''}`} className="flex items-center gap-2 rounded border border-mn-border/50 bg-black/15 px-2 py-1">
                    <span className="min-w-0 flex-1 truncate text-xs text-mn-text/90" title={check.name}>{check.name}</span>
              ...
              {prChecks.length > 5 && <p className="text-xs text-mn-muted">+{prChecks.length - 5} more check(s)</p>}
            </div>
          </div>
        )}
        {(setupCount > 0 || runCount > 0 || teardownCount > 0) && (
          <>
            <div className="mt-2 flex flex-wrap gap-1">
              {setupCount > 0 && <span className="rounded border border-mn-blue/20 bg-mn-blue/10 px-1.5 py-0.5 text-xs text-mn-blue">{setupCount} setup</span>}
              {runCount > 0 && <span className="rounded border border-mn-cyan/20 bg-mn-cyan/10 px-1.5 py-0.5 text-xs text-mn-cyan">{runCount} run/check</span>}
              {teardownCount > 0 && <span className="rounded border border-mn-yellow/20 bg-mn-yellow/10 px-1.5 py-0.5 text-xs text-mn-yellow">{teardownCount} teardown</span>}
            </div>
            <div className="mt-3 space-y-2 rounded-lg border border-mn-border/60 bg-black/10 p-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-mn-muted">Commands run in inspectable terminals and are written to activity.</span>
              </div>
              {config?.run.slice(0, 3).map((command, index) => (
                <div key={`${index}-${command}`} className="flex items-center justify-between gap-2 rounded border border-mn-border/50 bg-black/15 px-2 py-1">
                  <code className="min-w-0 truncate text-[11px] text-mn-muted" title={command}>{command}</code>
                  <Button
                    variant="secondary"
                    size="xs"
              ...
              {runCount > 3 && <p className="text-xs text-mn-muted">+{runCount - 3} more command(s) available in terminal tools.</p>}
              {actionMessage && <p className="text-xs text-mn-muted">{actionMessage}</p>}
            </div>
          </>
        )}
        {blockers.length > 0 && (
  ...
  return (
    <div className="px-4 pb-4">
      <div className="rounded-xl border border-mn-border bg-mn-card/70 p-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-mn-muted">Ship Flow</p>
            <p className="mt-0.5 text-xs text-mn-muted">Guided local path: review → checks → PR → cleanup.</p>
          </div>
        </div>
        <div className="space-y-1.5">
          {steps.map((step) => (
            <div key={step.label} className="flex items-center gap-2 rounded border border-mn-border/50 bg-black/10 px-2 py-1.5">
              {step.done ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-mn-cyan" /> : <Circle className="h-3.5 w-3.5 shrink-0 text-mn-muted" />}
              <span className="min-w-0 flex-1 text-xs font-medium text-mn-text/90">{step.label}</span>
              <span className="min-w-0 truncate text-right text-xs text-mn-muted" title={step.hint}>{step.hint}</span>
            </div>
          ))}
        </div>
        {prDraft && (
          <div className="mt-3 rounded-lg border border-mn-border/60 bg-black/10 p-2">
            <div className="mb-1.5 flex items-center justify-between gap-2">
              <p className="min-w-0 truncate text-xs font-semibold text-mn-text/85" title={prDraft.title}>
            ...
            <p className="line-clamp-3 text-xs leading-relaxed text-mn-muted">{prDraft.summary}</p>
            <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
              <CockpitLine label="Key changes" value={`${prDraft.keyChanges.length}`} />
              <CockpitLine label="Risks" value={`${prDraft.risks.length}`} />
            </div>
            <ul className="mt-2 list-disc space-y-0.5 pl-4 text-xs text-mn-muted">
...
export function LifecyclePanel({
  ...
  return (
    <div className="px-4 pb-4">
      <div className="rounded-xl border border-mn-border bg-mn-card/70 p-3">
        <div className="mb-2">
          <p className="text-xs font-semibold uppercase tracking-widest text-mn-muted">Lifecycle</p>
          <p className="mt-0.5 text-xs text-mn-muted">Explicit workspace actions with safe defaults and recoverable history.</p>
        </div>
        ...
        {workspaceSchedulerJobs.length > 0 && (
          <div className="mt-2 rounded border border-mn-border/60 bg-black/10 p-2">
            <p className="text-xs font-semibold text-mn-text/85">Scheduler jobs</p>
            <div className="mt-1 space-y-1">
              {workspaceSchedulerJobs.slice(0, 4).map((job) => {
                ...
                return (
                  <div key={job.id} className="rounded border border-mn-border/50 bg-black/15 px-2 py-1.5 text-xs">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate text-mn-text/85">{humanizeTaskLabel(job.kind)}</p>
                      ...
                      <span className="shrink-0 text-mn-muted">{formatSchedulerInterval(job.intervalSeconds)}</span>
                    </div>
                    <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                      ...
                      <span className="text-[11px] text-mn-muted">Existing durable job · no direct DB edits needed</span>
                    </div>
                  </div>
                );
              })}
              {workspaceSchedulerJobs.length > 4 && <p className="text-xs text-mn-muted">+{workspaceSchedulerJobs.length - 4} more scheduler job(s)</p>}
            </div>
            {schedulerMessage && <p className="mt-2 text-xs text-mn-muted">{schedulerMessage}</p>}
          </div>
        )}
        ...
        {unhealthySessions.length > 0 && (
          <div className="mt-2 space-y-1 rounded border border-mn-border/60 bg-black/10 p-2">
            {unhealthySessions.slice(0, 3).map((terminal) => (
              <div key={terminal.sessionId} className="rounded border border-mn-border/50 bg-black/15 px-2 py-1.5 text-xs">
                <div className="flex items-center justify-between gap-2">
                  <span className="min-w-0 truncate text-mn-text/85" title={terminal.title}>{terminal.title}</span>
                  <span className="shrink-0 text-mn-muted">{terminal.recommendedAction}</span>
                </div>
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  <span className="rounded border border-mn-border/50 bg-white/5 px-1 py-0.5 text-[10px] uppercase text-mn-muted">
            ...
            {unhealthySessions.length > 3 && <p className="text-xs text-mn-muted">+{unhealthySessions.length - 3} more session(s)</p>}
          </div>
        )}
        <div className="mt-3 flex flex-wrap gap-2">
          <Button variant="secondary" size="xs" disabled={cleanupBusy} onClick={onCleanup}>
            ...
            <div className="mt-1 space-y-1">
              {recoveryResult.actions.slice(0, 4).map((action) => (
                <div key={action.sessionId} className="flex items-start gap-2 text-xs">
                  ...
                  <span className="min-w-0 flex-1 text-mn-muted">
                    <span className="text-mn-text/85">{action.title}</span> — {action.reason}
                  </span>
                </div>
              ))}
              {recoveryResult.actions.length > 4 && <p className="text-xs text-mn-muted">+{recoveryResult.actions.length - 4} more action(s)</p>}
            </div>
          </div>
        )}
        {message && <p className="mt-2 text-xs text-mn-muted">{message}</p>}
