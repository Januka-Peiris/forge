...
import type {
  ActivityItem as MnemonicActivityItem,
...
interface DetailPanelProps {
  ...
  activityItems?: MnemonicActivityItem[];
...
export function DetailPanel({
  ...
  if (!workspace) {
    return (
      <aside className="w-[300px] shrink-0 h-full bg-mn-surface flex flex-col items-center justify-center">
        <div className="text-center px-6">
          <div className="w-10 h-10 rounded-xl bg-mn-card border border-mn-border flex items-center justify-center mx-auto mb-3">
            <Activity className="w-5 h-5 text-mn-muted" />
          </div>
          <p className="text-sm font-medium text-mn-muted">No workspace selected</p>
          <p className="text-sm text-mn-muted mt-1">Select a workspace to inspect it</p>
        </div>
      </aside>
    );
  }
  ...
  const activityRows = activityItems.slice(0, 8).map((item) => {
    const tone = item.level === 'error'
      ? { icon: AlertCircle, color: 'bg-mn-red/70' }
      : item.level === 'warning'
      ? { icon: AlertTriangle, color: 'bg-mn-yellow/70' }
      : item.level === 'success'
      ? { icon: CheckCircle2, color: 'bg-mn-cyan/70' }
      : { icon: Circle, color: 'bg-mn-muted/60' };
    return {
      icon: tone.icon,
      color: tone.color,
  ...
  return (
    <aside className="w-full shrink-0 h-full bg-mn-surface flex flex-col overflow-hidden">
      {/* Dynamic Header */}
      <div className="px-4 pt-4 pb-3 shrink-0 border-b border-mn-border/40 bg-mn-bg shadow-sm">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 mb-0.5">
              <h2 className="text-sm font-bold text-mn-text truncate">{workspace.name}</h2>
              {workspacePrStatus?.found && (
                <span className="text-xs font-mono text-mn-muted shrink-0">#{workspacePrStatus.number}</span>
              )}
            </div>
            <div className="flex items-center gap-1.5 text-[11px] text-mn-muted">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {onOpenInCursor && (
              <Button
                ...
                className="text-mn-muted hover:text-mn-blue hover:bg-mn-blue/10"
            ...
            {primaryAction && (
              <Button 
                ...
                className={primaryAction.variant === 'default' ? 'bg-mn-cyan hover:bg-mn-cyan-high text-white shadow-electric-glow' : ''}
              >
            ...
            {onCollapse && (
              <Button variant="ghost" size="icon-xs" onClick={onCollapse} className="text-mn-muted">
        ...
        <div className="flex items-center gap-3">
          {workspace.behindBy > 0 ? (
            <div className="flex items-center gap-1.5 text-xs font-semibold text-mn-orange">
              <AlertTriangle className="w-3.5 h-3.5" />
              Behind by {workspace.behindBy} commit{workspace.behindBy === 1 ? '' : 's'}
            </div>
          ) : workspacePrStatus?.found ? (
            <div className="flex items-center gap-1.5 text-xs font-semibold text-mn-cyan">
              <CheckCircle2 className="w-3.5 h-3.5" />
              {workspacePrStatus.checksSummary === 'success' ? 'All checks passed' : workspacePrStatus.state}
            </div>
          ) : (
            <div className="flex items-center gap-1.5 text-xs font-semibold text-mn-muted">
              <Circle className="w-3 h-3" />
              {changedFileCount > 0 ? `${changedFileCount} files changed` : 'Clean workspace'}
            </div>
          )}
          <div className="ml-auto flex items-center gap-1.5 text-[10px] text-mn-muted/60 uppercase tracking-wider">
            <span className="w-1.5 h-1.5 rounded-full bg-mn-cyan/40" />
            {sessionStatus}
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'status' | 'config')} className="flex flex-col flex-1 min-h-0">
        <TabsList className="px-4 bg-black/20 border-b border-mn-border/40">
