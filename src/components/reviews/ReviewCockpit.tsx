...
export function ReviewCockpit({
  ...
  if (!workspace) {
    return (
      <div className="flex flex-1 items-center justify-center p-8">
        <div className="max-w-sm rounded-xl border border-dashed border-mn-border p-8 text-center">
          <ClipboardList className="mx-auto mb-3 h-9 w-9 text-mn-muted" />
          <p className="text-ui-body font-semibold text-mn-text">No workspace selected</p>
          <p className="mt-1.5 text-ui-label leading-relaxed text-mn-muted">
            Select a workspace first, then open the review cockpit.
          </p>
          {onBackToWorkspaces && (
            <button
              type="button"
              onClick={onBackToWorkspaces}
              className="mt-4 inline-flex items-center gap-1.5 rounded-md border border-mn-border bg-mn-surface-overlay px-3 py-1.5 text-ui-caption font-semibold text-mn-text/90 hover:bg-mn-surface-overlay-high"
            >
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-mn-bg">
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="shrink-0 border-b border-mn-border bg-mn-surface/95 px-5 py-3 backdrop-blur">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            {onBackToWorkspaces && (
              <button
                type="button"
                onClick={onBackToWorkspaces}
                className="mb-2 inline-flex items-center gap-1.5 rounded-md border border-mn-border bg-mn-surface-overlay px-2.5 py-1 text-ui-caption font-semibold text-mn-text/90 hover:bg-mn-surface-overlay-high"
              >
                <ArrowLeft className="h-3.5 w-3.5" />
                Back to Workspaces
              </button>
            )}
            <h1 className="truncate text-ui-headline font-bold text-mn-text">Review Cockpit</h1>
            <p className="mt-0.5 truncate text-ui-label text-mn-muted">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-mn-border bg-mn-surface-overlay px-3 py-1.5 text-ui-label font-semibold text-mn-text hover:bg-mn-surface-overlay-high disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-mn-blue/25 bg-mn-blue/10 px-3 py-1.5 text-ui-label font-semibold text-mn-blue hover:bg-mn-blue/15 disabled:opacity-50"
            >
        ...
        <div className="mt-2.5 flex flex-wrap items-center gap-2">
          <Popover>
            <PopoverTrigger asChild>
              <button className="flex items-center gap-1.5 rounded-full border border-mn-border bg-mn-surface-overlay px-2 py-0.5 text-ui-caption font-semibold text-mn-text/80 transition-all hover:bg-mn-surface-overlay-high">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="flex items-center gap-1.5 rounded-full border border-mn-cyan/20 bg-mn-cyan/10 px-2 py-0.5 text-ui-caption font-semibold text-mn-cyan transition-all hover:bg-mn-cyan/15">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="flex items-center gap-1.5 rounded-full border border-mn-border bg-mn-surface-overlay px-2 py-0.5 text-ui-caption font-semibold transition-all hover:bg-mn-surface-overlay-high">
                <span className="flex items-center text-mn-green"><Plus className="h-2.5 w-2.5" />{totalAdds}</span>
                <span className="h-3 w-px bg-mn-border" />
                <span className="flex items-center text-mn-red"><Minus className="h-2.5 w-2.5" />{totalDels}</span>
              </button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-auto p-2 text-ui-label">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="flex items-center gap-1.5 rounded-full border border-mn-blue/20 bg-mn-blue/10 px-2 py-0.5 text-ui-caption font-semibold text-mn-blue transition-all hover:bg-mn-blue/15">
        ...
        {(error || (cockpit?.warnings.length ?? 0) > 0) && (
          <div className="mt-2 rounded-lg border border-mn-yellow/20 bg-mn-yellow/10 px-3 py-2 text-ui-label text-mn-yellow">
      ...
      >
        ...
        {/* ── Left: file list ─────────────────────────────────────────────── */}
        <div className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-mn-border bg-mn-card/70">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-mn-border/40">
            {!cockpit && (
              <p className="p-3 text-ui-label text-mn-muted">
                {busy ? 'Loading…' : 'No data yet.'}
              </p>
            )}
            {cockpit?.files.length === 0 && (
              <p className="p-3 text-ui-label text-mn-muted">No changed files.</p>
            )}
            {cockpit?.files.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-start gap-2 px-3 py-2.5 text-left transition-colors hover:bg-mn-surface-overlay ${
                    active ? 'bg-mn-cyan/10' : ''
                  }`}
                >
                  <div className="relative mt-0.5 shrink-0">
                    {reviewed ? (
                      <CheckCircle2 className="h-3.5 w-3.5 text-mn-cyan" />
                    ) : (
                      <FileCode
                        className={`h-3.5 w-3.5 ${active ? 'text-mn-cyan' : 'text-mn-muted'}`}
                      />
                    )}
                    {isRisky && (
                      <span className="absolute -right-0.5 -top-0.5 flex h-2 w-2">
                        <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-mn-red opacity-75"></span>
                        <span className="relative inline-flex h-2 w-2 rounded-full bg-mn-red"></span>
                      </span>
                    )}
                  </div>
                  <span className="min-w-0 flex-1">
                    <span
                      className={`block truncate font-mono text-ui-label ${
                        active ? 'text-mn-cyan' : 'text-mn-text/90'
                      }`}
                    >
                      {item.file.path}
                    </span>
                    <span className="mt-0.5 block text-ui-tiny uppercase tracking-wide text-mn-muted">
                      {item.file.status}
                    </span>
                  </span>
                  <span className="shrink-0 font-mono text-ui-caption leading-none mt-0.5">
                    <span className="text-mn-green">+{item.file.additions ?? 0}</span>
                    {' '}
                    <span className="text-mn-red">-{item.file.deletions ?? 0}</span>
                  </span>
                </button>
              );
            })}
        ...
        <div className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-mn-border bg-mn-card/70">
          ...
          <div className="shrink-0 flex items-center justify-between gap-3 border-b border-mn-border/60 px-3 py-2">
            <div className="min-w-0 flex-1 flex items-center gap-2">
              <p className="min-w-0 truncate font-mono text-ui-label font-semibold text-mn-text">
                {selectedFile?.file.path ?? 'No file selected'}
              </p>
              {(selectedFile?.file.path.includes('config') || selectedFile?.file.path.includes('.env')) && (
                <span className="shrink-0 flex items-center gap-1 px-1.5 py-0.5 rounded bg-mn-red/10 border border-mn-red/25 text-ui-tiny font-bold text-mn-red uppercase tracking-tighter">
            ...
            <div className="shrink-0">
              {selectedFile?.review?.status === 'reviewed' ? (
                <button
                  ...
                  className="flex items-center gap-1 rounded-md border border-mn-yellow/25 bg-mn-yellow/10 px-2.5 py-1 text-ui-caption font-semibold text-mn-yellow hover:bg-mn-yellow/15 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1 rounded-md bg-mn-cyan px-2.5 py-1 text-ui-caption font-bold text-white shadow-electric-glow hover:bg-mn-cyan-high disabled:opacity-50 transition-all"
                >
          ...
          <div className="shrink-0 flex flex-wrap items-center gap-1.5 border-b border-mn-border px-3 py-2 bg-mn-surface/40">
            ...
            <span className="mx-1 h-4 w-px bg-mn-border/60 shrink-0" />
            <button
              disabled={busy || !selectedFile}
              onClick={() => void sendPrompt('explain_diff')}
              className="flex items-center gap-1 rounded-md border border-mn-border bg-mn-surface-overlay px-2.5 py-1 text-ui-caption font-semibold text-mn-text/85 hover:bg-mn-surface-overlay-high disabled:opacity-50"
            >
        ...
        <div className="flex min-h-0 flex-col gap-2" style={{ display: rightPanelCollapsed ? 'none' : undefined }}>
          ...
          {/* Readiness card */}
          <div className="shrink-0 rounded-xl border border-mn-border bg-mn-card/70 p-3">
            <div className="mb-2 flex items-center justify-between gap-2">
              <p className="text-ui-caption font-bold uppercase tracking-wider text-mn-muted">Readiness</p>
              <button
                disabled={busy}
                onClick={() => void loadCockpit(true, effectiveSelectedPath)}
                className="flex items-center gap-1 rounded px-1.5 py-0.5 text-ui-caption text-mn-muted hover:bg-mn-surface-overlay hover:text-mn-text/90 disabled:opacity-50"
              >
            ...
            <div className="space-y-2">
              {/* File change counts */}
              <div className="flex items-center justify-between text-ui-caption text-mn-muted">
                <span>{cockpit?.files.length ?? 0} files changed</span>
                <span className="font-mono">
                  <span className="text-mn-green">+{totalAdds}</span>
                  <span className="ml-1 text-mn-red">-{totalDels}</span>
                </span>
              </div>
              ...
              {cockpit?.reviewSummary?.summary && (
                <p className="text-ui-caption leading-relaxed text-mn-muted line-clamp-3">
                  {cockpit.reviewSummary.summary}
                </p>
              )}
              {/* Merge readiness detail */}
              {cockpit?.mergeReadiness && (
                <div className="border-t border-mn-border/60 pt-2 text-ui-caption text-mn-muted space-y-0.5">
                  <div className="flex items-center justify-between">
                    <span>Ahead</span>
                    <span className="font-mono text-mn-cyan">{cockpit.mergeReadiness.aheadCount ?? '—'}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Behind</span>
                    <span className="font-mono text-mn-yellow">{cockpit.mergeReadiness.behindCount ?? '—'}</span>
                  </div>
                  ...
                  {!cockpit && (
                  <p className="text-ui-caption text-mn-muted">{busy ? 'Loading…' : 'No data yet.'}</p>
                  )}
                  </div>
                  </div>
                  ...
                  <div className="shrink-0 rounded-xl border border-mn-border bg-mn-card/70 p-3">
                  <p className="mb-2 text-ui-caption font-bold uppercase tracking-wider text-mn-muted flex items-center gap-1.5">
                  <ShieldCheck className="h-3 w-3" /> Pre-Flight Health
                  </p>
                  <div className="space-y-1.5">
                  {!cockpit?.mergeReadiness?.preFlightChecks?.length && (
                  <p className="text-ui-caption text-mn-muted italic">No automated checks run yet.</p>
                  )}
                  {cockpit?.mergeReadiness?.preFlightChecks.map((check) => {
                  const statusColor = check.status === 'pass' ? 'text-mn-cyan' : check.status === 'fail' ? 'text-mn-red' : 'text-mn-yellow';
                  const StatusIcon = check.status === 'pass' ? CheckCircle2 : check.status === 'fail' ? AlertCircle : Info;

                  return (
                  <div key={check.id} className="rounded-lg border border-mn-border/40 bg-black/10 p-2">
                    <div className="flex items-center justify-between gap-2 mb-0.5">
                      <span className="text-ui-caption font-bold text-mn-text/90">{check.label}</span>
                      <StatusIcon className={`h-3 w-3 ${statusColor}`} />
                    </div>
                    <p className="text-ui-tiny text-mn-muted leading-snug">{check.message}</p>
                  </div>
                  );
                  })}
                  </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-mn-border bg-mn-card/70">
            ...
            <div className="shrink-0 border-b border-mn-border/60 px-3 py-2 text-ui-caption text-mn-muted">
              {prCommentGroups.total === 0 ? (
                ...
                <span>
                  <span className="font-semibold text-mn-text/85">{prCommentGroups.openCount}</span> needing attention · {prCommentGroups.fileGroups.length} file group{prCommentGroups.fileGroups.length === 1 ? '' : 's'} · {prCommentGroups.general.length} general
                </span>
              )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-mn-border/40">
              {prCommentGroups.total === 0 ? (
                <p className="p-4 text-ui-body leading-relaxed text-mn-muted">
                  No PR comments cached yet. Use <span className="font-semibold text-mn-text">PR comments</span> in the header to fetch team or Greptile feedback.
                </p>
              ) : (
                <>
                  {prCommentGroups.fileGroups.map((group) => (
                    <div key={group.path}>
                      <button
                        ...
                        className={`flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-ui-caption font-bold uppercase tracking-wider hover:bg-mn-surface-overlay ${group.path === effectiveSelectedPath ? 'bg-mn-cyan/10 text-mn-cyan' : 'text-mn-muted'}`}
                      >
                        <span className="min-w-0 truncate">{group.path}</span>
                        <span className="shrink-0 rounded bg-mn-surface-overlay px-1.5 py-0.5 font-mono">{group.comments.length}</span>
                      </button>
                      <div className="divide-y divide-mn-border/40">
                  ...
                  {prCommentGroups.general.length > 0 && (
                    <div>
                      <div className="flex items-center justify-between gap-2 px-3 py-2 text-ui-caption font-bold uppercase tracking-wider text-mn-muted">
                        <span>General comments</span>
                        <span className="rounded bg-mn-surface-overlay px-1.5 py-0.5 font-mono">{prCommentGroups.general.length}</span>
                      </div>
                      <div className="divide-y divide-mn-border/40">
