    ...
    <Dialog open>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Mnemonic checked your local tools. Missing tools will not block app usage.</DialogDescription>
        </DialogHeader>
        <DialogBody className="space-y-2">
          {items.map((item) => (
            <div key={item.binary} className="rounded-xl border border-mn-border bg-mn-bg/80 px-3 py-2">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <span className={item.status === 'ok' ? 'text-mn-cyan' : item.status === 'missing' ? 'text-mn-red' : 'text-mn-yellow'}>
                    {item.status === 'ok' ? '✓' : item.status === 'missing' ? '✗' : '?'}
                  </span>
                  <span className="text-[13px] font-semibold text-mn-text">{item.name}</span>
                  {item.optional && <Badge variant="muted">optional</Badge>}
                </div>
                <span className="text-[10px] uppercase tracking-widest text-mn-muted">{item.status}</span>
              </div>
              {item.status !== 'ok' && (
                <div className="mt-2 flex items-center gap-2 rounded-lg border border-mn-border bg-black/20 px-2 py-1.5">
                  <span className="text-[11px] text-mn-muted">Run:</span>
                  <code className="flex-1 truncate text-[11px] text-mn-text">{item.fix}</code>
