...
export function FederatedTaskCockpit({ workspace, workspaces, repositories, relationships, onSelectWorkspace, onCreateWorkspaceForRepo }: FederatedTaskCockpitProps) {
  ...
  const readinessClass = readiness?.level === 'ready'
    ...
    : readiness?.level === 'blocked'
      ? 'border-forge-red/30 bg-forge-red/10 text-forge-red'
      : 'border-forge-yellow/30 bg-forge-yellow/10 text-forge-yellow';

  return (
    <section className="mx-2 mt-2 rounded-xl border border-forge-border bg-forge-card/75 p-3 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Network className="h-4 w-4 text-forge-orange" />
            <h2 className="text-sm font-bold text-forge-text">Federated task cockpit</h2>
            <span className="rounded border border-forge-border/70 bg-black/20 px-1.5 py-0.5 text-[10px] text-forge-muted">{group.members.length} repos</span>
            {activeArtifactCount > 0 && <span className="rounded border border-forge-blue/30 bg-forge-blue/10 px-1.5 py-0.5 text-[10px] text-forge-blue">{activeArtifactCount} artifact{activeArtifactCount === 1 ? '' : 's'} open</span>}
          </div>
          <p className="mt-1 text-xs text-forge-muted">Coordinate ship-together state across related repo workspaces.</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      {error && <p className="mt-2 rounded border border-forge-red/30 bg-forge-red/10 px-2 py-1 text-xs text-forge-red">{error}</p>}
      <div className="mt-3 grid gap-3 xl:grid-cols-[1fr_1fr]">
        <div className="space-y-2">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-forge-muted/70">Group members & PR set</p>
            ...
            {group.members.map((member) => {
              ...
              return (
                <div
                  ...
                  className={`rounded-lg border px-2 py-1.5 text-left hover:border-forge-orange/40 ${member.id === workspace.id ? 'border-forge-orange/40 bg-forge-orange/10' : 'border-forge-border/60 bg-black/10'}`}
                >
                  <div className="flex items-center gap-2"><span className="truncate text-xs font-semibold text-forge-text">{member.repo}</span><span className="ml-auto rounded bg-white/5 px-1.5 py-0.5 text-[10px] text-forge-muted">{member.status}</span></div>
                  <p className="mt-0.5 truncate font-mono text-[11px] text-forge-muted">{member.branch}</p>
                  <p className="mt-1 flex items-center gap-1 text-[11px] text-forge-muted"><GitPullRequest className="h-3 w-3" />{pr?.found ? (pr.url ? <a href={pr.url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="text-forge-blue hover:underline">PR #{pr.number ?? '—'} · {pr.checksSummary}</a> : `PR #${pr.number ?? '—'} · ${pr.checksSummary}`) : (pr?.warning ? 'PR unavailable' : 'No PR found')}</p>
          ...
          {warnings.length > 0 && (
            <div className="rounded-lg border border-forge-yellow/30 bg-forge-yellow/10 p-2">
              <div className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-forge-yellow"><AlertTriangle className="h-3.5 w-3.5" /> Companion work warnings</div>
              <div className="space-y-1">
                {warnings.slice(0, 3).map((warning) => (
                  <div key={`${warning.repoId}:${warning.relationshipKind}`} className="flex items-start gap-2 text-xs text-forge-muted">
          ...
          {readiness && (
            <div className="rounded-lg border border-forge-border/60 bg-black/10 p-2">
              <p className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-forge-text">{readiness.level === 'ready' ? <CheckCircle2 className="h-3.5 w-3.5 text-forge-green" /> : <AlertTriangle className="h-3.5 w-3.5 text-forge-yellow" />} Ship-together readiness</p>
              <ul className="list-disc space-y-0.5 pl-4 text-xs text-forge-muted">{readiness.reasons.slice(0, 4).map((reason) => <li key={reason}>{reason}</li>)}</ul>
            </div>
          )}
        </div>
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-forge-muted/70">Coordination artifacts</p>
            <select value={artifactStatusFilter} onChange={(event) => setArtifactStatusFilter(event.target.value as CoordinationArtifactStatus | 'all')} className="ml-auto rounded border border-forge-border bg-forge-card px-1 py-0.5 text-[11px] text-forge-text">
              <option value="all">all statuses</option>
              {STATUS_OPTIONS.map((status) => <option key={status} value={status}>{status}</option>)}
            </select>
            <select value={artifactKindFilter} onChange={(event) => setArtifactKindFilter(event.target.value as CoordinationArtifactKind | 'all')} className="rounded border border-forge-border bg-forge-card px-1 py-0.5 text-[11px] text-forge-text">
              <option value="all">all kinds</option>
              {KIND_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
            </select>
          </div>
          <div className="max-h-52 space-y-1.5 overflow-y-auto pr-1">
            {artifacts.length === 0 && <p className="rounded-lg border border-dashed border-forge-border/60 px-2 py-2 text-xs text-forge-muted">No artifacts yet.</p>}
            {artifacts.length > 0 && visibleArtifacts.length === 0 && <p className="rounded-lg border border-dashed border-forge-border/60 px-2 py-2 text-xs text-forge-muted">No artifacts match the current filters.</p>}
            {visibleArtifacts.map((artifact) => (
              <div key={artifact.id} className="rounded-lg border border-forge-border/60 bg-black/10 p-2">
                <div className="flex items-start gap-2">
                  <div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-forge-text">{artifact.title}</p><p className="mt-0.5 text-[11px] text-forge-muted">{artifact.artifactKind.replace(/_/g, ' ')} · {artifact.targetWorkspaceId ? `target ${group.members.find((member) => member.id === artifact.targetWorkspaceId)?.repo ?? artifact.targetWorkspaceId}` : 'group-wide'}</p></div>
                  <select value={artifact.status} onChange={(event) => void changeArtifactStatus(artifact, event.target.value as CoordinationArtifactStatus)} className="rounded border border-forge-border bg-forge-card px-1 py-0.5 text-[11px] text-forge-text">{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{status}</option>)}</select>
                  <Button type="button" variant="ghost" size="icon-xs" onClick={() => editArtifact(artifact)}><Pencil className="h-3 w-3" /></Button>
                  <Button type="button" variant="ghost" size="icon-xs" onClick={() => void deleteArtifact(artifact)}><Trash2 className="h-3 w-3 text-forge-red" /></Button>
                </div>
                {artifact.body && <p className="mt-1 line-clamp-2 text-xs text-forge-text/80">{artifact.body}</p>}
              </div>
            ))}
          </div>
          <div className="rounded-lg border border-forge-border/60 bg-black/10 p-2">
            <div className="mb-2 flex items-center justify-between"><p className="text-xs font-semibold text-forge-text">{form.editingId ? 'Edit artifact' : 'Add artifact'}</p>{form.editingId && <Button type="button" variant="ghost" size="icon-xs" onClick={resetForm}><X className="h-3 w-3" /></Button>}</div>
            <div className="grid grid-cols-2 gap-1.5">
              <select value={form.artifactKind} onChange={(event) => setForm((current) => ({ ...current, artifactKind: event.target.value as CoordinationArtifactKind }))} className="rounded border border-forge-border bg-forge-card px-2 py-1 text-xs text-forge-text">{KIND_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
              <select value={form.targetWorkspaceId} onChange={(event) => setForm((current) => ({ ...current, targetWorkspaceId: event.target.value }))} className="rounded border border-forge-border bg-forge-card px-2 py-1 text-xs text-forge-text"><option value="">Group-wide</option>{group.members.map((member) => <option key={member.id} value={member.id}>{member.repo}</option>)}</select>
              <input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="Title" className="col-span-2 rounded border border-forge-border bg-forge-card px-2 py-1 text-xs text-forge-text placeholder:text-forge-muted/50" />
              <textarea value={form.body} onChange={(event) => setForm((current) => ({ ...current, body: event.target.value }))} placeholder="Handoff details…" rows={2} className="col-span-2 resize-none rounded border border-forge-border bg-forge-card px-2 py-1 text-xs text-forge-text placeholder:text-forge-muted/50" />
              <select value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value as CoordinationArtifactStatus }))} className="rounded border border-forge-border bg-forge-card px-2 py-1 text-xs text-forge-text">{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{status}</option>)}</select>
