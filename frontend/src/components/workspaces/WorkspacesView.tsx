      ...
      <div className="px-6 pt-6 pb-4 border-b border-forge-border shrink-0">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-[12px] text-forge-muted mt-1.5">Parallel AI coding workspaces tied to repos, branches, and worktrees</p>
          </div>
          <button
            onClick={onNewWorkspace}
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-forge-orange hover:bg-orange-500 text-[13px] font-semibold text-white transition-colors shadow-lg shadow-orange-900/30"
          >
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 relative max-w-xs">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 bg-forge-card border border-forge-border rounded-lg text-[12px] text-forge-text placeholder:text-forge-muted/80 focus:outline-none focus:border-forge-blue/50 transition-colors"
            />
          </div>
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-7 py-2 bg-forge-card border border-forge-border rounded-lg text-[12px] text-forge-text focus:outline-none focus:border-forge-blue/50 cursor-pointer transition-colors"
            >
              <option>All</option>
        ...
        <div className="flex items-center gap-2 mt-3">
          {Object.entries(statusCounts).map(([status, count]) => (
            <span key={status} className="text-[11px] text-forge-muted">
              <span className="text-forge-text font-medium">{count}</span> {status}
            </span>
          ))}
          <span className="text-forge-muted text-[11px]">·</span>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-8">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-center">
            <p className="text-forge-muted text-[13px]">No workspaces match your filter</p>
            <p className="text-forge-muted text-[12px] mt-1">Try adjusting your search or filter</p>
          </div>
        ) : (
